...
%>
	...
	<br>
	<br>
		<h2 style="color: green;  text-align:center; font-size: 30px;">Thank you for Writing Online Exam</h2><br>
		<%
			
		if(user.getStatus().equals("Qualified")){ %>
		<h3 style="color: green; text-align:center;"><b>Congratulations <%=user.getName() %> U r Qualified in <%=user.getTechnology() %>Exam, </b></h3><br>
			
		<h3 style="text-align:center;">Total Questions:   <%=user.getLimitquestions() %><br><br>
		Correct Answers :      <%=user.getMarks() %><br><br>
		Incorrect Answers:      <%=user.getLimitquestions()-user.getMarks() %><br><br>
		Results:      Qualified<br></h3>
		<%}else{ %>
